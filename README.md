...
A partitioned refresh will copy and rename a template M query partition named "Template" for each new partition. The template partition must have "-1" and "-2" placeholders, which will be replaced with the partition's Minimum and Maximum from the JSON definition respectively.
...
### Run
Create an "activity.json" configuration file using templates provided above in the same directory as the executable and run "AnalysisServicesRefresh.exe".
...
# Contribute
Please feel free to raise any issues or pull requests to help improve this product.