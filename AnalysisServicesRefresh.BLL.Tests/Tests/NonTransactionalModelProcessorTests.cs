...
{
    [TestClass]
    public class NonTransactionalModelProcessorTests
    {
        ...
        public void Setup()
        {
            ...
            _sut = new NonTransactionalModelProcessor(
        ...
        public async Task TestLogsException()
        {
            ...
            _logger.Verify(x => x.Error(It.Is<Exception>(y => y.Message == "Test exception.")));
        }
        ...
        [TestMethod]
        public async Task TestContinuesProcessingIfTableProcessingThrowsException()
        {
        ...
        public async Task TestRecreatesResourcesForEachTable()
        {
            ...
            _server.Verify(x => x.Connect("ConnectionString"), Times.Exactly(5));
            _server.Verify(x => x.Disconnect(), Times.Exactly(5));
            _server.Verify(x => x.Dispose(), Times.Exactly(5));
            _connectionStringFactory.Verify(x => x.Create(It.IsAny<string>()), Times.Exactly(5));
        }
        ...
        {
            await _sut.ProcessAsync(_configuration);
            _logger.Verify(x => x.Info($"Saving {_configuration.FullTables.First().Name}."), Times.Once);
            _logger.Verify(x => x.Info($"Saving {_configuration.PartitionedTables.First().Name}."), Times.Once);
        }
        ...
        [TestMethod]
        public async Task TestLogsFailedTablesIfTableProcessingThrowsException()
        {
            _model.Setup(x => x.SaveChanges(It.IsAny<SaveOptions>()))
                .Throws<Exception>();
            ...
            _logger.Verify(x => x.Error(It.Is<Exception>(y =>
                y.Message ==
                "Non-transactional processing failed for the following tables: FullTableName, PartitionedTableName.")));
        }

        [TestMethod]
        public async Task TestLogsFailedTablesExceptionsIfTableProcessingThrowsException()
        {
            _model.Setup(x => x.SaveChanges(It.IsAny<SaveOptions>()))
